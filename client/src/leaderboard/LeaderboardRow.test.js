...
describe('LeaderboardRow', () => {
    it('renders name', () => {
        ...
        expect(row.findByProps({className: "leader-name"}).children[0]).toBe(name)
    });
    ...
    it('renders weeks of 0', () => {
        ...
        expect(row.findByProps({className: "leader-correct-weeks"}).children[0]).toBe("0")
    });

    it('renders weeks of 1', () => {
        const row = create(<LeaderboardRow name="Gandalf" weeks={1} picks={0}/>).root;
        expect(row.findByProps({className: "leader-correct-weeks"}).children[0]).toBe("1")
    });

    it('renders picks of 0', () => {
        const row = create(<LeaderboardRow name="Gandalf" weeks={0} picks={0}/>).root;
        expect(row.findByProps({className: "leader-correct-picks"}).children[0]).toBe("0")
    });

    it('renders picks of 1', () => {
        const row = create(<LeaderboardRow name="Gandalf" weeks={1} picks={1}/>).root;
        expect(row.findByProps({className: "leader-correct-picks"}).children[0]).toBe("1")
    });
});